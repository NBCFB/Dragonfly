...
func CreateListener(address string) (net.Listener, error) {
	// Create a TCP listener, as http listener is a TCP listener
...
func CreateOrImportListener(address string) (net.Listener, error) {
	...
	// Since no listener to be imported, create a listener at the first
...
func ForkChild(address string, listener net.Listener) (*os.Process, error) {
	// Parse the listener metadata and pass it to the child in the environment.
	listenerFile, err := GetListenerFile(listener)

	log.Printf("Get listener file at %v", listenerFile.Name())
	if err != nil {
		return nil, err
	}
	defer listenerFile.Close()
